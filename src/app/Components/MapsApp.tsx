    ...
    <div className="content">
      <div className="map-content flex flex-col gap-6 h-full">
        ...
        >
          ...
          {eventsData.filter((event) =>!selectedCategory || event.category === selectedCategory)
            .map((event) => {
              return (
                <Marker
                  key={event.id}
          ...
          {activeEvent && (
            <FlyToMarker position={activeEvent.position} zoomLevel={14} />
          )}
        </MapContainer>
      </div>
      ...
      <div className="liked-events">
        <h2 className="liked-events__title">
          <i className="fa-solid fa-star"></i>Favourite&nbsp;Restaurants
        </h2>
        <ul>
          {favourites
            .map((id) => {
